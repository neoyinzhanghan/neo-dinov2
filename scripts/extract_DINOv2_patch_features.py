...
from dinov2.deploy.DINOv2FeatureExtractor import load_model
...
group.add_argument("--flatten", default=False, action="store_true", help="Flatten the model output.")
...
print("{} WSIs were missing mpp values: {}".format(len(bad_fpths_no_mpp), bad_fpths_no_mpp))
...
if args.guess_largest_batch_size:
    ...
    print("Guessing the best batch size took {} seconds".format(guest_batch_size_runtime))
...
for wsi_fpath in tqdm(wsi_fpaths, desc="WSI"):
    try:
        ...
        wsi_feats_save_fpath = os.path.join(args.save_dir, "{}-patch_features.h5".format(wsi_name))
        wsi_patch_folder = os.path.join(args.patch_dir, wsi_name)

        # load already computed patch grid
        patch_coords, patch_grid_idx = load_patch_grid_coords(folder=wsi_patch_folder, max_n=args.max_n_patches)
        ...
        #########################
        start_time = time()
        add_array_batch_to_h5(fpath=wsi_feats_save_fpath, data=features, name="features")
        add_array_batch_to_h5(fpath=wsi_feats_save_fpath, data=patch_coords, name="coords")
        add_array_batch_to_h5(fpath=wsi_feats_save_fpath, data=patch_grid_idx, name="patch_grid_idx")
        ...
        if args.continue_on_error_with_log is None:
            ...
            error_log_fpath = os.path.join(args.continue_on_error_with_log, "{}.txt".format(wsi_name))
